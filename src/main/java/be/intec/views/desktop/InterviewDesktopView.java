...
import be.intec.models.binders.InterviewBinder;
import be.intec.models.entities.InterviewEntity;
import be.intec.repositories.InterviewRepository;
import be.intec.services.exceptions.InterviewException;
...
public class InterviewDesktopView extends Application {

	final HBox actBox = new HBox();
	final HBox navBox = new HBox();
	private final TableView< InterviewBinder > table = new TableView<>();
	private final ObservableList< InterviewBinder > data = FXCollections.observableArrayList();
	...
	public void start( Stage stage ) {
		...
		stage.setTitle( "Interview Desktop View" );
		stage.setWidth( 500 );
		stage.setHeight( 650 );

		final Label header = new Label( "Interview Records" );
		...
		column0.setCellValueFactory(
				new PropertyValueFactory< InterviewBinder, Integer >( "id" ) );
		column0.setEditable( false );
		TableColumn column1 = new TableColumn( "startTime" );
		column1.setCellValueFactory(
				new PropertyValueFactory< InterviewBinder, java.sql.Timestamp >( "startTime" ) );
		column1.setEditable( true );
		column1.setOnEditCommit( ( EventHandler< CellEditEvent< InterviewBinder, java.sql.Timestamp > > ) t -> {
		...
		TableColumn column2 = new TableColumn( "endTime" );
		column2.setCellValueFactory(
				new PropertyValueFactory< InterviewBinder, java.sql.Timestamp >( "endTime" ) );
		column2.setEditable( true );
		column2.setOnEditCommit( ( EventHandler< CellEditEvent< InterviewBinder, java.sql.Timestamp > > ) t -> {
		...
		column3.setCellValueFactory(
				new PropertyValueFactory< InterviewBinder, Integer >( "applicationId" ) );
		column3.setEditable( true );
		column3.setOnEditCommit( ( EventHandler< CellEditEvent< InterviewBinder, Integer > > ) t -> {
		...
		addButton.setOnAction( onClick -> {
			...
			try {
				int noOfNewRecords = repository.save(
						new InterviewEntity()
								.withParsedStartTime( column1EditField.getText() )
								.withParsedEndTime( column2EditField.getText() )
				...
				if ( noOfNewRecords > 0 ) {
					...
					List< InterviewEntity > itemList = repository.findAll();

					for ( InterviewEntity item : itemList ) {
						data.add( new InterviewBinder(
								item.getId(),
								item.getStartTime(),
								item.getEndTime(),
		...
		viewButton.setOnAction( onClick -> {
			...
			message.setText( "Interview records are refreshed" );
