...
import be.intec.models.binders.OrganizationBinder;
import be.intec.models.entities.OrganizationEntity;
import be.intec.repositories.OrganizationRepository;
import be.intec.services.exceptions.OrganizationException;
...
public class OrganizationDesktopView extends Application {

	final HBox actBox = new HBox();
	final HBox navBox = new HBox();
	private final TableView< OrganizationBinder > table = new TableView<>();
	private final ObservableList< OrganizationBinder > data = FXCollections.observableArrayList();
	...
	public void start( Stage stage ) {
		...
		stage.setTitle( "Organization Desktop View" );
		stage.setWidth( 500 );
		stage.setHeight( 650 );

		final Label header = new Label( "Organization Records" );
		...
		column0.setCellValueFactory(
				new PropertyValueFactory< OrganizationBinder, Integer >( "id" ) );
		column0.setEditable( false );
		TableColumn column1 = new TableColumn( "code" );
		column1.setCellValueFactory(
				new PropertyValueFactory< OrganizationBinder, String >( "code" ) );
		column1.setEditable( true );
		column1.setOnEditCommit( ( EventHandler< CellEditEvent< OrganizationBinder, String > > ) t -> {
		...
		column2.setCellValueFactory(
				new PropertyValueFactory< OrganizationBinder, String >( "name" ) );
		column2.setEditable( true );
		column2.setOnEditCommit( ( EventHandler< CellEditEvent< OrganizationBinder, String > > ) t -> {
		...
		column3.setCellValueFactory(
				new PropertyValueFactory< OrganizationBinder, String >( "description" ) );
		column3.setEditable( true );
		column3.setOnEditCommit( ( EventHandler< CellEditEvent< OrganizationBinder, String > > ) t -> {
		...
		addButton.setOnAction( onClick -> {
			...
			try {
				...
				if ( noOfNewRecords > 0 ) {
					...
					List< OrganizationEntity > itemList = repository.findAll();

					for ( OrganizationEntity item : itemList ) {
						data.add( new OrganizationBinder(
								item.getId(),
								item.getCode(),
								item.getName(),
								item.getDescription()
		...
		viewButton.setOnAction( onClick -> {
			...
			message.setText( "Organization records are refreshed" );
